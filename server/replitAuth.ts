...
async function upsertUser(claims: any) {
  ...
  const replitId = claims["sub"]?.toString() ?? "";
  const email = claims["email"] ?? null;
  const username = claims["first_name"] 
    ? `${claims["first_name"]}${claims["last_name"] ? " " + claims["last_name"] : ""}`
    : email ?? `user${replitId}`;
  const profileImage = claims["profile_image_url"] ?? null;
...
export const isAdmin: RequestHandler = async (req, res, next) => {
  ...
  try {
    const replitId = user.claims.sub.toString();
